    ...
    def calculate(self):
        ...
        if redn_n == 2:
            for i in range(redn_n):
                L1, L2, A1x1, A2x2 = self.listofTMSpan[i].CalculateAx()
                L1_Mat.append(L1)
                L2_Mat.append(L2)
                L1_L2_Mat.append(2*(L1 + L2))
                b.append(-6*(A1x1/L1 + A2x2/L2))

            np.fill_diagonal(L_Mat[1:], L1_Mat)
            np.fill_diagonal(L_Mat, L1_L2_Mat)
            np.fill_diagonal(L_Mat[:, 1:], L2_Mat)
            L_Mat[1, 0] = L1_Mat[1]
...
solve = QsSolver([A, B, C, D, E, F, H, G], [supportA, supportB, supportC, supportD],
